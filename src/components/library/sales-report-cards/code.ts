export const salesReportCardsCode = `'use client';
      ...
      {candles.map((c, i) => {
        ...
        return (
          <div key={i} style={{ flex: 1, position: 'relative', height: '100%' }}>
            ...
            <motion.div
              style={{
                position: 'absolute', left: 0, right: 0, bottom: norm(bodyLow),
          ...
          {Array.from({ length: total }).map((_, si) => (
            <motion.div
              ...
              style={{
                ...
                background: si < row.filled ? 'linear-gradient(90deg, #16a34a, #34d399)' : 'rgba(255,255,255,0.07)',
    ...
    <div ref={ref} style={{ display: 'flex', gap: 5, height: 80, alignItems: 'flex-end' }}>
      {bars.map((h, i) => (
        <div key={i} style={{ flex: 1, position: 'relative', height: '100%', display: 'flex', alignItems: 'flex-end' }}>
          <motion.div
            style={{
              position: 'absolute', right: -4, bottom: 0, width: 4,
  ...
  return (
    <div style={{
      ...
      <motion.div
        initial={{ opacity: 0, y: 28 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.55, delay: 0.34, ease: [0.25, 0.46, 0.45, 0.94] }}
        whileHover={{ y: -4, boxShadow: '0 16px 48px rgba(52,211,153,0.14)' }}
        style={{
          width: '100%', maxWidth: 480,
          ...
          borderRadius: 18, padding: '22px 24px',
          display: 'flex', alignItems: 'center', justifyContent: 'space-between',
          gap: 20, cursor: 'pointer', overflow: 'hidden',
        }}
      >
